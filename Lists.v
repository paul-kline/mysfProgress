...
Fixpoint oddmembers (l:natlist) : natlist :=
 match l with
  | nil => nil
  | h :: d => if (oddb h) then h :: (oddmembers d) else oddmembers d 
 end. 
...
Inductive MyPair {T:Type} :=
 mypair : T -> T -> MyPair. 


Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match (mypair l1 l2) with 
   | mypair nil nil => nil
   | mypair l nil => l
   | mypair nil l => l 
   | mypair (h1 :: d1) (h2::d2) => h1 :: h2 :: (alternate d1 d2)
  end. 
...
Fixpoint count (v:nat) (s:bag) : nat := 
  match s with
   | nil => O
   | h :: d => if (beq_nat h v) then S (count v d) else count v d
  end. 
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with 
   | nil => nil
   | h :: d => if (beq_nat h v) then d else h :: (remove_one v d)
  end. 
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with 
   | nil => nil
   | h :: d => if (beq_nat h v) then remove_all v d else h :: (remove_all v d)
  end. 
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with 
   | nil => true
   | h :: d => andb (member h s2) (subset d (remove_one h s2))
  end.
(* very wrong definition of 'subset' in my mind' *) 
...
Theorem stupidTheorem : forall n : nat, forall b : bag,  S (count n b) =  (count n (add n b)).
Proof. intros. simpl. SearchAbout beq_nat. rewrite <- beq_nat_refl.
reflexivity. 
Qed.
(** [] *)
...
Theorem theorem1 : forall n : nat, forall ls : natlist, rev (snoc ls n) = n :: (rev ls).
Proof. intros. induction ls .
  simpl.  reflexivity. 
  simpl. rewrite IHls.  simpl. reflexivity. 
Qed.   

Theorem rev_involutive : forall l : natlist,
  rev (rev l) = l.
Proof. intros. induction l.  reflexivity. 
 simpl. SearchAbout snoc. rewrite theorem1.  rewrite IHl. reflexivity. 
Qed.  
...
Proof. intros. SearchAbout "++". rewrite app_assoc.  rewrite app_assoc.  reflexivity. 
Qed. 
...
Proof. intros. induction l1. 
  simpl. SearchAbout "++".  rewrite app_nil_end. reflexivity.
  simpl. rewrite IHl1. simpl. SearchAbout snoc. rewrite snoc_append. rewrite snoc_append. 
...
Proof. intros. induction l1. 
  simpl.  reflexivity.
  SearchAbout nonzeros. destruct n. 
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match mypair l1 l2 with 
    ...
    | mypair _ nil => false
    | mypair nil _ => false
    | mypair (x::t1) (y::t2) => andb (beq_nat x y) (beq_natlist t1 t2)
  end. 
...
Proof. intros. induction l.  simpl. reflexivity.
 simpl. SearchAbout beq_nat.  rewrite <- beq_nat_refl. simpl.  rewrite IHl. reflexivity. 
Qed. 
...
Theorem cons_snoc_app : forall x : nat, forall l1 : natlist, 
   x :: (l1 ++ [x]) = snoc ([x] ++ l1) x.
Proof. intros. simpl. SearchAbout "++". rewrite snoc_append. reflexivity. 
Qed.
(* pretty trival actually.. *)
...
Proof. intros. induction s. simpl. reflexivity. 
  simpl. destruct n.  simpl. rewrite ble_n_Sn. reflexivity.   
...
Print sum. Check sum. 
Theorem bag_count_sum : forall n : nat, 
                        forall b : bag, 
                        2 * (count n b) = count n (sum b b). 
Proof. intros. induction b.  simpl. reflexivity. 
simpl. case_eq (beq_nat n0 n). intros. simpl. simpl in IHb. rewrite <- IHb. rewrite plus_n_Sm.  
   rewrite plus_0_r. rewrite plus_comm.  reflexivity. 
   intros.  simpl.  rewrite <- IHb.  simpl.  reflexivity. 
Qed.
...
SearchAbout snoc. 
Theorem snoc_notEmpty : forall n : nat, forall l :natlist, snoc l n <> [].
Proof. intros.  simpl.  induction l.  simpl. unfold not. intros.  inversion H. 
 simpl.  unfold not.  intros. unfold not in IHl. inversion H. 
Qed.

Theorem helper : forall l : natlist, rev l = [] -> l = [].
Proof. intros. induction l . reflexivity. inversion H. contradiction snoc_notEmpty with n (rev l).
Qed.   

Theorem rev_injective : forall (l1 l2 : natlist), rev l1 = rev l2 -> l1 = l2.
Proof. intros. SearchAbout rev. rewrite <- rev_involutive. rewrite <- H. rewrite rev_involutive.  reflexivity. Qed. 

(* wow. waste of time. it is easy if you think... 
   induction l1 ,l2. reflexivity. 
 symmetry in H. apply helper in H.  rewrite H. reflexivity. apply helper in H.  rewrite H. reflexivity. 
simpl.  simpl in H. SearchAbout snoc. rewrite snoc_append in H. rewrite snoc_append in H. SearchAbout "++".  
rewrite <- distr_rev in H.  .  

 simpl.   as [|n l1']. simpl in H. symmetry in H.  apply helper in H. symmetry. exact H. 
induction l2 as [|x l2'].  simpl.  apply helper in H.  exact H. inversion H.   
*)
(* FILL IN HERE *)
(** [] *)
...
Proof. simpl. intros.  rewrite <- beq_nat_refl. reflexivity. 
Qed. 
(** [] *)
